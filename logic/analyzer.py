...
def analyze_expenses(total_spent, category_sums):
    report = []
    advice = []
    potential_savings = 0
    ...
    for category, amount in category_sums:
        ...
        if norm_range:
            ...
            if percentage > max_norm_pct:
                ...
                advice.append(f"• По категории *{category}* превышение нормы. Можно сэкономить около {int(excess_amount):,} AMD.")
            elif percentage > (max_norm_pct * 0.9):
                status_emoji = "🟡"
        
        report.append(f"{status_emoji} {category}: {int(amount):,} AMD ({percentage:.1f}%)")
